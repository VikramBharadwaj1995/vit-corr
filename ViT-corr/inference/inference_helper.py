    ...
    def one_pass(model, img_a, img_b):
        ...
        if LARGE_GPU:
            try:
                ...
                out = model.forward(img, queries, 'normal')['pred_corrs'].detach().cpu().numpy()[0]
        ...
        out_grid = out_grid[0]
        out_grid = out_grid.permute(1, 0, 2, 3)
        ...
        # print("cycle_grid: ", cycle_grid.shape)
...
def cotr_corr_base(model, img_a, img_b, queries_a):
    def one_pass(model, img_a, img_b, queries):
        ...
        out = model.forward(img, queries, 'normal')['pred_corrs'].clone().detach()
        cycle = model.forward(img, out[0], 'cycle')['pred_corrs'].clone().detach()

        queries = queries.cpu().numpy()[0]
        out = out.cpu().numpy()[0][0]
        cycle = cycle.cpu().numpy()[0][0]
        conf = np.linalg.norm(queries - cycle, axis=1, keepdims=True)
        #print("Shapes - ")
        #print("queries: ", queries.shape)
        #print("out: ", out.shape)
        #print("cycle: ", cycle.shape)
