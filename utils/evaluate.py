...
def calculate_epe_and_pck_per_dataset(test_dataloader, network, device, threshold_range, path_to_save=None,
    ...
    for j in range(len(test_dataloader.__dict__['dataset'].__dict__['df'])):
        string = test_dataloader.__dict__['dataset'].__dict__['df'][j]['source_image']+"->"+test_dataloader.__dict__['dataset'].__dict__['df'][j]['target_image']+'.npy'
        corr_list.append(string)
